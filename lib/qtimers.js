...
// timeoutHeap stores lists of setTimeout and setInterval tasks
var timeoutHeap = new Heap({compar: function(a, b){ return a._timeout - b._timeout}});

// timeoutHash accesses existing timeout queues
var timeoutHash = {};
...
TimeoutItem.prototype.ref = function() { if (!this._isref) { this._isref = 1; pendingTimeouts += 1; } }
TimeoutItem.prototype.unref = function() { if (this._isref) { this._isref = 0; pendingTimeouts -= 1; } }
...
function _scheduleTimeoutItem( item, when ) {
    ...
    if (!bucket) {
        bucket = timeoutHash[when] = {_timeout: when, _list: new Array()};
        // TODO: no need to put the entire bucket on the heap,
        // TODO: only need the timeout; bucket is always on hash.
        timeoutHeap.put(bucket);
    }
    bucket._list.push(item);
...
function _processTimeoutTasks( ) {
    ...
    var i, item, bucket, nbuckets = 0;
    while ((bucket = timeoutHeap.peek()) && bucket._timeout <= timestamp) {
        timeoutHeap.get();
        // TODO: put only timeouts on heap, get bucket from hash
        delete timeoutHash[bucket._timeout];
        var refCount = _processTasklist(bucket._list);
