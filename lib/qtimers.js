...
function TimeoutItem( fn, av ) { this._callback = fn; this._argv = av; }
TimeoutItem.prototype = {_callback: null, _argv: null, _func: null, _domain: null, _isref: 1, _when: 0, _interval: 0, ref: refItem, unref: unrefItem};
function timeoutItemTemplate( fn, av ) {
    return {_callback: fn, _argv: av, _func: null, _domain: null, _isref: 1, _when: 0, _interval: 0, ref: refItem, unref: unrefItem}; }
...
function _createTimeoutItem( fn, ms, argv ) {
    var item = new TimeoutItem(fn, argv);
...
function _invokeCallbackItem( item ) {
    if (item._domain) if (item._domain.disposed) return; else item._domain.enter();

    var err = _tryCallbackItem(item);

    // return on error without exiting domain, let caller re-throw in the called domain
    if (err) return err;
...
function _processTasklist( list ) {
    ...
    for (var i=0; i<list.length; i++) {
        ...
        var err = _tryCallback(item._callback, item._argv);
        if (err) {
...
function _getImmediateLimit( ) {
    ...
    // FIXME: as implemented, counts list entries, not runnable tasks
}

/*
 * called as needed from setImmediate to run immediate tasks
 * Runs some or all waiting tasks, optionally some newly queued ones too.
 */
function _processImmediateTasks( limit ) {
    ...
    while (n < maxDepth && (item = list.shift())) {
        ...
        var err = _tryCallbackItem(item);
        if (err) {
            // finish running the other immediate tasks if the error was not fatal
...
// fetch the oldest list queued tasks with triggered timers
function _fetchTimeoutTasklist( heap, timestamp ) {
    var when = heap.shift();
    var whenKey = when + '';
    var list = heap[whenKey];
    delete heap[whenKey];
    return list;
}
 ...
 * Processes all timeouts whose trigger time has arrived.
 */
function _processTimeoutTasks( ) {
    var ts = setTimestamp();

    while (timeoutHeap.peek() <= ts) {
        var when = timeoutHeap.shift();
        var whenKey = when + '';
        var list = timeoutHash[whenKey];
        delete timeoutHash[whenKey];
        //var list = _fetchTimeoutTasklist(timeoutHeap, ts);
    ...
    scheduleTimer(timeoutHeap.peek(), ts);
}
